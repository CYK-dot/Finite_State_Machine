...
void FSM_Set_Startup_Status(RCS_FSM_T* fsm,FNCT_VOID_PVD startup_state,FSM_EVENT_t startup_event)
{
    ...
    {
        #ifdef FSM_DEBUG
            RCS_Shell_Logs("ERROR:FSM.c,setup startup state earlier than state-function was registered!");
        #endif
    }
...
FSM_Num_t FSM_Find_State_ID(RCS_FSM_T* fsm,FNCT_VOID_PVD State)
{
    ...
    return -1;
    #ifdef FSM_DEBUG
        RCS_Shell_Logs("Error:Can`t Find Target State");
    #endif
}
...
FSM_Num_t FSM_Find_Action_ID(RCS_FSM_T* fsm,FNCT_VOID_PVD Action)
{
    ...
    #ifdef FSM_DEBUG
        RCS_Shell_Logs("Error:Can`t Find Target Action");
    #endif
}
...
static void FSM_Assert(RCS_FSM_T* fsm)
{
    ...
    {
        #ifdef FSM_DEBUG
        RCS_Shell_Logs("Error:FSM Action or State function overflow!");
        uint8_t err=10/0.0f;
        #endif
    }
    return;
...
static FSM_Num_t FSM_State_Trans_Assert(RCS_FSM_T* fsm)
{
    ...
    {
        return 0;
        RCS_Shell_Logs("Warning:FSM.c,jump to an undefined state!");
    }
    else 
        return 1;
}
...
{
    #ifdef FSM_DEBUG
    RCS_Shell_Logs("Error:FSM Entered error state or action");
    #endif
